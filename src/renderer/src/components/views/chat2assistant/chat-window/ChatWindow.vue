  ...
  <div class="chat-window">
          ...
          <a-dropdown :align-point="true" trigger="contextMenu">
            ...
            <div :id="`chat-message-${msg.id}`" class="chat-message"
              ...
              <div class="chat-message-avatar">
                ...
                <AssistantAvatar v-else-if="msg.role === 'assistant'" :provider="currentAssistant.provider"
                  :size="30" />
              </div>
              <!-- 消息内容 -->
    ...
    <div class="chat-input-container">
      ...
      <div class="chat-input-tools">
        ...
        <!-- 发音开始/停止 -->
        <!-- <a-tooltip v-if="isSupportSpeech(currentAssistant.provider)" :content="$t('chatWindow.startSpeech')"
          position="tr" mini :content-style="{ fontSize: '12px' }">
          <a-button v-if="speechStatus === SpeechStatus.STOP" size="mini" shape="round" style="margin-left: auto"
            @click="startSpeech(currentAssistant.chatMessageList.at(-1)?.content)">
            <icon-sound :size="15" />
          </a-button>
          <a-button v-else size="mini" shape="round" status="danger" style="margin-left: auto" @click="stopSpeech()">
